...
R tries to coerce variables of a different type (i.e. not numeric or logical) to NAs. Since the character vector is not based on numbers/integers, we are unable to convert to numeric. Because of that, I will try to convert the character variable into a factor variable, and then into a numeric variable:
...
as.factor(pull(prob1_df, vec_logical)) * pull(prob1_df, random_sample) #The console says "not meaningful for factors" and displays a string of NAs because the factor variable I created contained letters.
